...
def compute_eer(labels, scores):
    ...
    fpr, tpr, thresholds = roc_curve(labels, np.array(scores), pos_label=0)
    fnr = 1 - tpr
...
def main():
    ...
    #calculate the FPR and FNR rate by inferring the threshold at which EER occurs for each user
    #then testing infected keystrokes against a classifier with that threshold
    ...
    #     gallery = embeddings[uid][:5]
    #     queries = embeddings[uid][:10]
    #     test_queries = embeddings_infected[uid][10:SEQUENCES_PER_USER] if args.attack else embeddings[uid][10:SEQUENCES_PER_USER]
    #     #skip users without enough samples
    #     if len(queries) == 0:
    #         continue
    #     for q in queries:
    ...
    #         # genuine
    #         genu = [np.linalg.norm(g - q) for g in gallery]
    #         test_scores.append(np.mean(genu)); test_labels.append(1)
    #     potential_indices = np.array(range(5))
    #     potential_test_indices = np.array(range(len(test_queries)))
    #     for imp in users:
    #         impostor_queries = embeddings[imp][5:10]
    #         impostor_query = impostor_queries[np.random.choice(potential_indices,replace=True)]
    #         impostor_test_queries = embeddings_infected[imp][10:SEQUENCES_PER_USER] if args.attack else embeddings[imp][10:SEQUENCES_PER_USER]
    #         impostor_test_query = impostor_test_queries[np.random.choice(potential_test_indices,replace=True)]
    #         if imp == uid: continue
    #         #imp_gal = embeddings[imp][:G]
    #         impd = [np.linalg.norm(g - impostor_query) for g in gallery]
    #         #impd = [np.linalg.norm(g - q) for g in gallery]
    #         scores.append(np.mean(impd)); labels.append(0)
    #         impd_test = [np.linalg.norm(g - impostor_test_query) for g in gallery]
    #         test_scores.append(np.mean(impd_test)); test_labels.append(0)
    #     eer, eer_threshold = compute_eer(labels, scores)
    #     avg_eer += eer
    #     avg_threshold += eer_threshold
    #     test_scores_arr = np.array(test_scores)
    #     test_labels_arr = np.array(test_labels)
    #     test_scores_arr = (test_scores_arr<=eer_threshold).astype(int)
    #     true_positives= (test_labels_arr==1).sum()
    #     true_negatives = (test_labels_arr==0).sum()
    #     false_positives = ((test_scores_arr-test_labels_arr)==1).sum()
    #     false_negatives = ((test_scores_arr-test_labels_arr)==-1).sum()
    #     FPR = false_positives/true_negatives
    #     FNR = false_negatives/true_positives
    #     avg_FPR+= FPR
    #     avg_FNR+= FNR
    ...
    # print(f"Unique labels: {set(labels)}")
    # #eer, eer_threshold = compute_eer(labels, scores)
    # print(f"k={args.k}, AVG_EER = {avg_eer*100:.2f}% AVG_Threshold = {avg_threshold},\
    #  AVG_FPR = {avg_FPR*100:.2f}%, AVG_FNR = {avg_FNR*100:.2f}%")
    """**************************************"""
    avg_FPR = 0
    avg_FNR = 0
    avg_eer = 0 
    ...
    print(f"Unique labels: {set(labels)}")
    #eer, eer_threshold = compute_eer(labels, scores)
    print(f"k={args.k}, EER = {eer*100:.2f}% Threshold = {eer_threshold}")
    ...
    #     avg_eer += eer
    #     avg_threshold += eer_threshold
    #     user_thresholds.append(eer_threshold)
    ...
    # median_threshold = np.median(np.array(user_thresholds))
    ...
    for uid in users:
        ...
        test_scores_arr = np.array(test_scores)
        test_labels_arr = np.array(test_labels)
        #test_scores_arr = (test_scores_arr<=avg_threshold).astype(int)
        test_scores_arr = (test_scores_arr<=eer_threshold).astype(int)
        true_positives= (test_labels_arr==1).sum()
        true_negatives = (test_labels_arr==0).sum()
        false_positives = ((test_scores_arr-test_labels_arr)==1).sum()
